    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_technique, name, description, GROUP_CONCAT(name_niveau) as GenresFilms FROM  t_avoir_niveau_technique_niveau
                                                            RIGHT JOIN t_technique ON t_technique.id_technique = t_avoir_niveau_technique_niveau.fk_technique
                                                            LEFT JOIN t_niveau ON t_niveau.id_niveau = t_avoir_niveau_technique_niveau.fk_niveau
                                                            GROUP BY id_technique"""
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING id_technique= %(value_id_film_selected)s"""
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            ...
            lst_data_film_selected = [item['id_technique'] for item in data_genre_film_selected]
            ...
            lst_data_genres_films_non_attribues = [item['id_niveau'] for item in data_genres_films_non_attribues]
            ...
            lst_data_genres_films_old_attribues = [item['id_niveau'] for item in data_genres_films_attribues]
            ...
            lst_data_genres_films_non_attribues = [item['name_niveau'] for item in data_genres_films_non_attribues]
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            strsql_insert_genre_film = """INSERT INTO t_avoir_niveau_technique_niveau (id_avoir_niveau_technique_niveau, fk_niveau, fk_technique)
            ...
            strsql_delete_genre_film = """DELETE FROM t_avoir_niveau_technique_niveau WHERE fk_niveau = %(value_fk_genre)s AND fk_technique = %(value_fk_film)s"""
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT id_technique, name, description, GROUP_CONCAT(id_niveau) as GenresFilms FROM t_avoir_niveau_technique_niveau
                                        INNER JOIN t_technique ON t_technique.id_technique = t_avoir_niveau_technique_niveau.fk_technique
                                        INNER JOIN t_niveau ON t_niveau.id_niveau = t_avoir_niveau_technique_niveau.fk_niveau
                                        WHERE id_technique = %(value_id_film_selected)s"""



        strsql_genres_films_non_attribues = """SELECT id_niveau, name_niveau FROM t_niveau WHERE id_niveau not in(SELECT id_niveau as idGenresFilms FROM t_avoir_niveau_technique_niveau
                                                    INNER JOIN t_technique ON t_technique.id_technique = t_avoir_niveau_technique_niveau.fk_technique
                                                    INNER JOIN t_niveau ON t_niveau.id_niveau = t_avoir_niveau_technique_niveau.fk_niveau
                                                    WHERE id_technique = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_technique, id_niveau, name_niveau FROM t_avoir_niveau_technique_niveau
                                            INNER JOIN t_technique ON t_technique.id_technique = t_avoir_niveau_technique_niveau.fk_technique
                                            INNER JOIN t_niveau ON t_niveau.id_niveau = t_avoir_niveau_technique_niveau.fk_niveau
                                            WHERE id_technique = %(value_id_film_selected)s"""
