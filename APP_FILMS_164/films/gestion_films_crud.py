...
def film_update_wtf():
    ...
    try:
        ...
        if form_update_film.validate_on_submit():
            ...
            desc_tech_update = form_update_film.desc_technique_update.data

            valeur_update_dictionnaire = {"value_id_film": id_film_update,
                                          "value_name_update" : nom_film_update,
                                          "value_tech_update": desc_tech_update
                                          }
            print("valeur_update_dictionnaire ", valeur_update_dictionnaire)

            str_sql_update_nom_film = """UPDATE t_technique SET name = %(value_name_update)s,
                                                                    description = %(value_tech_update)s
                                                                    WHERE id_technique = %(value_id_film)s"""
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_film = "SELECT * FROM t_technique WHERE id_technique = %(value_id_film)s"
            ...
            form_update_film.nom_film_update_wtf.data = data_film["name"]
            form_update_film.desc_technique_update.data = data_film["description"]
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_avoir_niveau_technique_niveau WHERE fk_technique = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_technique WHERE id_technique = %(value_id_film)s"""
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT * FROM t_technique WHERE id_technique = %(value_id_film)s"""
